...
const redis = new Redis(process.env.REDIS_URL + "?family=0" || 'redis://localhost:6379');
...
export default function socketHandler(io: Server) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('send_message', async (data: ChatMessage) => {
      ...
      const { room_id, message, personality_id } = data;
      ...
      if (isAIChat) {
        ...
        const personality = getPersonalityById(personality_id || 'therapist');
        ...
        const botReply = await getGeminiResponse(message, contextMessages, personality.id);
